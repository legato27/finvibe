...
| `get_scalp_desk` | read | Every symbol × setup (A continuation, B sweep-and-reclaim fade, C funding reversion) at the newest minute, ranked: fired first, then by how many of the setup's conditions hold, with each condition's verdict, the gates passed or failed (spread cap, fresh book, funding blackout, positive expected edge), the levels and net edge it would sign, the active signals and the risk budget remaining today. Same shape philosophy as the options desk: hard gates, then a score. | `symbol`?: string, `limit`?: integer |
