...
| `get_engine_scorecard` | read | Every option recommendation the engine logged, graded at expiry: win rate, captured premium, annualised return, assignment rate, mean predicted probability and the calibration gap, overall and by strategy, model agreement and days to expiry. | `window_days`?: integer |
